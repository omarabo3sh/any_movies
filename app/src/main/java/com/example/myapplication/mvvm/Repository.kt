...
class Repository @Inject constructor(val apiService: ApiService, val dao: MyDao) {
    ...
    suspend fun getMovies(): List<MovieResult>? { // this is the main function that
        ...
        val data = getMovieRemote(startPage = 1, endPage = 50)
        return data
    }
    ...
    private suspend fun getMovieRemote(startPage: Int, endPage: Int): List<MovieResult>? {

        return try {
            val data = mutableListOf<MovieResult>()
            for (i in startPage..endPage) {
                val pageData = apiService.getMovies(i)
                data.addAll(pageData.results ?: emptyList())
            }
            insert(data)
            data
        } catch (_: Exception) {
