...
import com.example.myapplication.data.nework.RemoteDataSource
...
object MyModule {
    ...
    fun provideOkhttp(): OkHttpClient {   // create an instance of OkHttpClient
        return OkHttpClient.Builder()
            .addInterceptor { chain -> // add an interceptor to the OkHttpClient
    ...
    @Singleton
    @Provides
    fun getRemoteDataSource(apiService: ApiService): IRemoteDataSource { // create an instance of the remote data source
        return RemoteDataSource(apiService) // return the remote data source
    }
    ...
    ): IRepository { // create an instance of the repository
        return Repository(iLocalDataSource, iRemoteDataSource) // return the repository
    }

    @Singleton
    @Provides
    fun getLocalDataSource(myDao: MyDao): ILocalDataSource { // create an instance of the local data source
        return LocalDataSource(myDao) // return the local data source
    }
